package org.example.structuredConcurrency.assignment;

import org.example.structuredConcurrency.LongRunningTask;
...
public class STaskScopeCustomExecutor {
    ...
    public static void main(String[] args) throws Exception {
        ...
        LongRunningTask.TaskResponse result
     ...
     */
    private static LongRunningTask.TaskResponse execute(Collection<LongRunningTask> tasks) throws Exception {
        
        try(var scope = new AverageWeatherTaskScope()) {
        
            // start running the tasks in parallel 
            List<Subtask<LongRunningTask.TaskResponse>> subTasks
            ...
            LongRunningTask.TaskResponse output = scope.response();
...
class AverageWeatherTaskScope extends StructuredTaskScope<LongRunningTask.TaskResponse> {
    
    private final List<Subtask<? extends LongRunningTask.TaskResponse>> responses
    ...
    private synchronized void add(Subtask<? extends LongRunningTask.TaskResponse> subtask) {
    ...
    public LongRunningTask.TaskResponse response() {
        ...
        LongRunningTask.TaskResponse r1 = responses.get(0).get();
        LongRunningTask.TaskResponse r2 = responses.get(1).get();
        ...
        return new LongRunningTask.TaskResponse("Weather", "" + (temp1 + temp2) / 2, avgTime);
    }
